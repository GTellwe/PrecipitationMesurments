...
class CapsNet(nn.Module):
    ...
    def loss(self, data, x, target, reconstructions):
        return self.quantile_loss(x, target,self.tau) + self.reconstruction_loss(data, reconstructions)
        '''
    ...
    def skewed_absolute_error(self, output, target, tau):
        """
    ...
    def quantile_loss(self, output, target, taus):
        """
        ...
        for i, tau in enumerate(taus[1:]):
            #print(tau)
            #print(i)
            e += self.skewed_absolute_error(torch.flatten(output[:, i + 1]),
                                       torch.flatten(target),
                                       tau)
        return e
